...
class MemoryErrorChecker {
    ...
    fun calculateRowDivisionChecksum(input: String): Int {
        val values = input.split(" ", "\t").map { it.toInt() }.sortedDescending()
        values.indices.forEach { i ->
            (i + 1..values.lastIndex)
                    .filter { values[i] % values[it] == 0 }
                    .forEach { return values[i] / values[it] }
        }
        return 0
    }
...
fun main(args: Array<String>) {
    ...
    with(MemoryErrorChecker()) {
        println("Captcha 1 : ${calculateArrayChecksum(input)}")
        println("Captcha 2 : ${calculateArrayDivisionChecksum(input)}")
    }
}